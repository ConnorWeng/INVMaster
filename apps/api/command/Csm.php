...
class Csm extends Command
{
    ...
    protected function buildService(Input $input, Output $output)
    {
        ...
        //将$id也填到数组
        $serviceParams['id'] = $this->getPk($model);

        //将model类名也填到数组
        $serviceParams['modelClassname'] = $modelClassname;

        if (is_file($serviceParams['file'])) {
            $output->writeln('<error>' . $serviceParams['classname'] . " Service already exists!</error>\r\n");
            return true;
        }
     ...
     * @return  [type]          [description]
     */
    protected function getPk($model)
    {
        $tableName = preg_replace("/([A-Z])/", "_\\1", $model);
        ...
        try {
            ...
            throw new Exception('Error : Model has no related datatable !! ');
        }
        ...
        if ($result) {
            foreach ($result as $key => $val) {
                ...
                if (!empty($info[$val['field']]['primary'])) {
                    $pk[] = $val['field'];
                }
            }
        }
    ...
    protected function buildErrorCode(Input $input, Output $output)
    {
        ...
        if (is_file($serviceParams['code_file'])) {
            ...
            // 如果存在则，追加错误码
            if(!$this->checkErrorCodeContent($serviceParams)){
                file_put_contents($serviceParams['code_file'], $this->rebuildErrorCodeFile($serviceParams));
            }
            
            $output->writeln("<info> \^o^/ Error_Code_File update successfully. \^o^/ </info>\r\n");
     ...
     * @return  [type]           [description]
     */
    protected function rebuildErrorCodeFile($params)
    {
        $patch = $this->getErrorCodePatch($params);

        $file = $params['code_file'];

        $content = file_get_contents($file);

        return str_replace(["/*{%new_code%}*/"], [$patch], $content);
     ...
     * @return  [type]           [description]
     */
    protected function checkErrorCodeContent($params)
    {
        $file = $params['code_file'];

        $content = require $file;

        return array_key_exists($params['code_prefix'], $content);
    }
     ...
     * @return  [type]  [description]
     */
    protected function getErrorCodePatch($params)
    {
        $patch = __DIR__ . '/stubs/patch/error_code.patch';

        $patch = file_get_contents($patch);

        $code_prefix = $params['code_prefix'];

        return str_replace(['{%code%}'], [$code_prefix], $patch);
    }
    ...
    {
        $params = "\r\nCommand : [ php think csm uri model ]\r\n";
        ...
        if (!$flag) {
            $params .= "\r\nWarning : 2 parameters [ uri , model ] are required !! \r\n";
        }
