...
class UsersIdService extends InnerService
{
    ...
    public $defaultParams = [
        'get' => [
            "users" => ['指定的用户资源', 0, PARAM_REQUIRED],
        ],
    ];
    ...
    public static function instance($params = [])
    {
        if (self::$instance == null) {
            self::$instance         = new UsersIdService();
...
/*-----------------------------------------------------------------------------------------------------------*/
    ...
    public function get()
    {
        $userId = $this->params['users'];
        $userId = (int)$userId;
        if(isset($userId)){
            $data = InvUser::get($userId);
            if($data){
                $data = $data->toArray();
