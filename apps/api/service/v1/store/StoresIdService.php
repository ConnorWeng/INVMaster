...
class StoresIdService extends InnerService
{
    ...
    public $defaultParams = [
        'get' => [
            "stores" => ['指定的店铺资源', 0, PARAM_REQUIRED],
        ],
    ];
    ...
    public static function instance($params = [])
    {
        if (self::$instance == null) {
            self::$instance         = new StoresIdService();
...
/*-----------------------------------------------------------------------------------------------------------*/
    ...
    public function get()
    {
        $storeId = $this->params['stores'];
        $storeId = (int)$storeId;
        if(isset($storeId)){
            $data = InvStore::get($storeId);
            if($data){
                $data = $data->toArray();
